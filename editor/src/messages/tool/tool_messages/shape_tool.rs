...
impl ShapeToolData {
	...
	pub fn shape_tool_modifier_keys() -> [Key; 3] {
	...
	fn transition(
		...
		match (self, event) {
			...
			(
				...
				if let ShapeToolFsmState::Drawing(ShapeType::QrCode) = self {
					QrCode::finalize(document, input, viewport, tool_data, responses);
				}
